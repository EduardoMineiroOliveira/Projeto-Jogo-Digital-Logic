...
//=============================================================================
 /*:
 * @plugindesc v1.35b Tenha mais controle sobre of fluxo do sistema de
 * batalha com esse plugin e altere vários aspectos ao seu gosto.
 ...
 *
 * @param Action Speed
 * @desc Essa é a fórmula usada para a velocidade da base de uma ação.
 * Padrão: agi + Math.randomInt(Math.floor(5 + agi / 4))
 * @default agi
 *
 * @param Default System
 * @desc Esse é o sistema de batalha padrão que seu jogo usa.
 * Padrão: dtb
 * @default dtb
 *
 * @param ---Escape---
 ...
 *
 * @param Escape Ratio
 * @desc Fórmula usada para determinar o sucesso do escape.
 * Padrão: 0.5 * $gameParty.agility() / $gameTroop.agility()
 * @default 0.5 * $gameParty.agility() / $gameTroop.agility()
 *
 * @param Fail Escape Boost
 * @desc Que o jogador falhar em tentar escapar, aumenta a
 * taxa de sucesso por essa quantidade. Padrão: 0.1
 * @default 0.1
 *
 * @param ---Animation---
 * @default
 ...
 * @desc Isso estabelece a base do delay entre animações.
 * Padrão: 8
 * @default 0
 *
 * @param Animation Next Delay
 * @desc Isso estabelece o delay sequencial entre animações.
 * Padrão: 12
 * @default 0
 *
 * @param Certain Hit Animation
 * @desc Animação padrão usada para certas habilidades de acerto.
 * Use 0 se você não quiser animação.
 * @default 120
 *
 * @param Physical Animation
 * @desc Animação padrão usada para habilidades físicas.
 * Use 0 se você não quiser animação.
 * @default 52
 *
 * @param Magical Animation
 * @desc Animação padrão usada para habilidades mágicas.
 * Use 0 se você não quiser animação.
 * @default 51
 *
 * @param Enemy Attack Animation
 * @desc Essa é a animação de ataque padrão usada por inimigos.
 * Padrão: 0
 * @default 39
 *
 * @param Reflect Animation
 * @desc A animação usada quando ataques mágicos são refletidos.
 * @default 42
 *
 * @param Motion Waiting
 * @desc Executa animações depois de realizar uma ação ou durante?
* Durante - false     Depois - true     Padrão: false
 * @default false
 *
 * @param ---Frontview---
 * @default
 ...
 * @desc Essa fórmula determina a posição de casa X do personagem.
 * Padrão: 0
 * @default Graphics.boxWidth / 8 + Graphics.boxWidth / 4 * index
 *
 * @param Front Position Y
 * @desc Essa fórmula determina a posição de casa Y do personagem.
 * Padrão: 0
 * @default Graphics.boxHeight - 180
 *
 * @param Front Actor Sprite
 * @desc Mostrar o sprite de batalha do personagem em vista de frente?
 * NÃO - false     SIM - true     Padrão - false
 * @default false
 *
 * @param Front Sprite Priority
 * @desc Dar aos sprites do personagem prioridade topo?
 * 0 - Normal   1 - Personagens no Topo   2 - Inimigos no Topo
 * @default 1
 *
 * @param ---Sideview---
 * @default
 ...
 * @desc Valor padrão usado para o Anchor X do seu sprite.
 * Padrão: 0.5
 * @default 0.5
 *
 * @param Default Y Anchor
 * @desc Valor padrão usado para o Anchor Y do seu sprite.
 * Padrão: 1.0
 * @default 1.0
 *
 * @param Step Distance
 * @desc Essa é a distância uma unidade anda pra frente para ações.
 * Padrão: 48
 * @default 48
 *
 * @param Flinch Distance
 * @desc Na vista de lado, quando uma unidade leva dano ou esquiva, ela irá
 * recuar uma certa distância em pixels.
 * @default 12
 *
 * @param Show Shadows
 * @desc Você deseja ter sombras aparecendo embaixo dos personagens?
NÃO - false    SIM - true
 * @default true
 *
 * @param ---Damage Popups---
 ...
 * @desc Ajusta quantos frames um popup ficará visível.
 * Padrão: 90
 * @default 128
 *
 * @param Newest Popup Bottom
 * @desc Posiciona o popup mais recente na parte de baixo de um grupo.
 * NÃO - false     SIM - true
 * @default true
 *
 * @param Popup Overlap Rate
 * @desc Quando múltiplos popups de dano aparecem, eles cobres eles mesmos.
 * Use isso para mudar a quantidade de ritmo de buffer para cada sprite.
 * @default 0.9
 *
 * @param Critical Popup
 * @desc Ajusta a cor piscante de popups para acertos críticos.
 * Padrão: 255, 0, 0, 160
 * @default 255, 0, 0, 160
 *
 * @param Critical Duration
 * @desc Quantos frames o piscamento irá durar para um crítico.
 * Padrão: 60
 * @default 60
 *
 * @param ---Tick-Settings---
 ...
 *
 * @param Timed States
 * @desc Se for baseado em Tick's, usar tempo em vez de rodadas
 * para estados? NÃO - false   SIM - true
 * @default true
 *
 * @param Timed Buffs
 * @desc Se for baseado em Tick's, usar tempo em vez de rodadas
 * para buffs? NÃO - false   SIM - true
 * @default true
 *
 * @param Turn Time
 * @desc Quantos ticks devem passar para serem iguais a 1 rodada?
 * @default 100
 * 
 * @param AI Self Turns
 ...
 *
 * @param Lower Windows
 * @desc Posiciona as janelas de habilidade e de item na parte de baixo
 * da tela. DESLIGADO - false     LIGADO - true
 * @default true
 *
 * @param Window Rows
 * @desc Para as janelas mais baixas, quantas linhas de itens você
 * quer que as janelas mostrem?
 * @default 4
 *
 * @param Command Window Rows
 * @desc Estabelece o número de linhas para cada janela de comando
 * para ser mostrado. Padrão: 4
 * @default 4
 *
 * @param Command Alignment
 * @desc Estabelece o alinhamento de texto para os comandos da
 * Party/Personagem. Padrão: left
 * @default center
 *
 * @param Start Actor Command
 * @desc Começa a rodada com a Janela de Comando do Personagem em vez da
 * Party. DESLIGADO - false     LIGADO - true
 * @default true
 *
 * @param Current Max
 * @desc Mostrar todo o valor de HP/MP atual/máximo?
 * NÃO - false     SIM - true     Padrão: true
 * @default false
 *
 ...
 * @desc Quando selecionando personagens e inimigos, mostrar a janela de
 * ajuda? NÃO - false     SIM - true
 * @default true
 *
 * @param User Help Text
 * @desc A forma singular de 'User' usada em uma janela de ajuda.
 * @default User
 *
 * @param Ally Help Text
 * @desc A forma singular de 'Ally' usada em uma janela de ajuda.
 * @default Ally
 *
 * @param Allies Help Text
 * @desc A forma plural de 'Allies' usada em uma janela de ajuda.
 * @default Allies
 *
 * @param Enemy Help Text
 * @desc A forma singular de 'Enemy' usada em uma janela de ajuda.
 * @default Enemy
 *
 * @param Enemies Help Text
 * @desc A forma plural de 'Enemies' usada em uma janela de ajuda.
 * @default Enemies
 *
 * @param All Help Text
 * @desc Quando selecionando um grupo inteiro de alvos.
 * %1 - Target Group (Allies or Enemies)
 * @default All %1
 *
 * @param Random Help Text
 * @desc Quando selecionando uma seleção aleatória de alvos.
 ...
 * @desc Substitui a tela de seleção do inimigo com uma mais visual.
 * DESLIGADO - false     LIGADO - true
 * @default true
 *
 * @param Show Enemy Name
 * @desc Mostra os nomes dos inimigos com o Visual Enemy Select.
 * DESLIGADO - false     LIGADO - true
 * @default true
 *
 * @param Enemy Font Size
 * @desc Muda o tamanho da fonte usada para mostrar os nomes dos inimigos.
 * Padrão: 28
 * @default 20
 *
 * @param Enemy Auto Select
 * @desc Muda qual inimigo é automaticamente selecionado primeiro.
 ...
 * @desc Permite a você clicar no personagem da tela para selecioná-lo.
 * DESLIGADO - false     LIGADO - true
 * @default true
 *
 * @param ---Battle Log---
 ...
 * @desc Mostra o texto de começo de batalha para inimigos aparecendo.
 * DESLIGADO - false     LIGADO - true
 * @default false
 *
 * @param Show Pre-Emptive Text
 * @desc Mostra o texto quando conseguir um ataque preventivo.
 * DESLIGADO - false     LIGADO - true
 * @default true
 *
 * @param Show Surprise Text
 * @desc Mostra o texto quando conseguir um ataque surpresa.
 * DESLIGADO - false     LIGADO - true
 * @default true
 *
 * @param Optimize Speed
 * @desc Corta o processo de linha log base para otimizar a velocidade
 * da batalha. DESLIGADO - false     LIGADO - true
 * @default true
 *
 * @param Show Action Text
 * @desc Mostra o texto de toda a ação ou um versão simplificada dela.
 * SIMPLES - false     INTEIRO - true
 * @default false
 *
 * @param Show State Text
 * @desc Mostra todo o texto em relação a estados.
 * DESLIGADO - false     LIGADO - true
 * @default false
 *
 * @param Show Buff Text
 * @desc Mostra todo o texto em relação a buffs.
 * DESLIGADO - false     LIGADO - true
 * @default false
 *
 * @param Show Counter Text
 * @desc Mostra o texto em relação a contra-ataques.
 * DESLIGADO - false     LIGADO - true
 * @default true
 *
 * @param Show Reflect Text
 * @desc Mostra o texto em relação a feitiços refletidos.
 * DESLIGADO - false     LIGADO - true
 * @default true
 *
 * @param Show Substitute Text
 * @desc Mostra o texto em relação a danos substituídos.
 * DESLIGADO - false     LIGADO - true
 * @default true
 *
 * @param Show Fail Text
 * @desc Mostra o texto em relação a ataques que falharam.
 * DESLIGADO - false     LIGADO - true
 * @default false
 *
 * @param Show Critical Text
 * @desc Mostra o texto em relação a acertos críticos.
 * DESLIGADO - false     LIGADO - true
 * @default false
 *
 * @param Show Miss Text
 * @desc Mostra o texto em relação a ataques que não acertaram.
 * DESLIGADO - false     LIGADO - true
 * @default false
 *
 * @param Show Evasion Text
 * @desc Mostra o texto em relação a ataques evadidos.
 * DESLIGADO - false     LIGADO - true
 * @default false
 *
 * @param Show HP Text
 * @desc Mostra o texto em relação a dano de HP ou curas.
 * DESLIGADO - false     LIGADO - true
 * @default false
 *
 * @param Show MP Text
 * @desc Mostra o texto em relação a dano de MP ou curas.
 * DESLIGADO - false     LIGADO - true
 * @default false
 *
 * @param Show TP Text
 * @desc Mostra o texto em relação a dano de TP ou curas.
 * DESLIGADO - false     LIGADO - true
 * @default false
 *
 * @help
 ...
 * ============================================================================
 *
 * Esse plugin altera os vários aspectos do sistema de batalha padrão,
 * permitindo-o que seja mais simplificado como os RPG's modernos e menos
 * desajeitado como os RPG's antigos. Isso inclui em escolher que texto irá
 * aparecer na janela de log da batalha no topo e como ele será mostrado.
 *
 * ============================================================================
 * Mensagens de Batalha
 * ============================================================================
 *
 * Quando mudando "Termos" e as "Mensagens" que aparecem na batalha, inserindo
 * o seguinte tag em qualquer lugar da mensagem irá causar com que ela se
 * centralize no log da batalha.
 *
 *   <CENTER>
 *   Esse tag tem que estar todo em letras maiúsculas para que a janela
 *   de log da batalha reconheça-a como uma instrução para centralizar a
 *   mensagem mostrada do texto de batalha.
 *
 * ============================================================================
 * Janelas de Batalha
 * ============================================================================
 *
 * Há várias opções para ajustar as configurações de janela achadas no
 * sistema de batalhas para fazer a navegação do sistema de batalha mais
 * intuitivo. Isso inclui começar rodadas com a Janela de Comando do
 * Personagem em vez de Janela de Comando da Party (a Janela de
 * Lutar/Escapar). A Janela de Comando da Party ainda é acessível mas
 * apenas se for pressionado "cancelar" na janela do primeiro
 * personagem.
 *
 * ============================================================================
 * Ordem da Batalha
 * ============================================================================
 *
 * A ordem de rodada da batalha também é consertada. Dessa forma, qualquer
 * combatente que tenha o valor de seu AGI mudado durante o curso da batalha
 * irá refletir nessas mudanças durante a rodada atual em vez de na próxima
 * rodada. A calculação da velocidade da ação também pode ser ajustada e
 * afinada para ter o fator aleatório da fórmula de sua calculação de
 * velocidade removida, também, fazendo com que AGI seja realmente um
 * parâmetro tático que vale a pena.
 *
 ...
 * ============================================================================
 *
 * Ações de Multi-acertos não irão mais terminar prematuramente se o alvo
 * morrer no meio da ação. Isso é feito por toggling estados imortais. Para
 * se fazer uso disso, tenha certeza que seu banco de dados tenha um
 * Immortal State em algum lugar. Se você não quiser fazer uso disso,
 * estabeleça o parâmetro do Immortal State ID para 0.
 *
 * ============================================================================
 * Revamp do Popup
 * ============================================================================
 *
 * Embora os popups de danos ainda podem parecer com os mesmos dos padrões
 * do MV, o processo em que eles são criados é agora diferente para
 * simplificar o processo de popup de dano. Antes, os popups só apareciam
 * um de cada vez com os frames diferentes no mínimo para eles mostrarem.
 * Agora, qualquer ação que ocorrer no mesmo frame irá mostrar todos os
 * popups do mesmo frame, fazendo com que os popups fiquem mais suaves
 * e menos desajeitados.
 *
 * ============================================================================
 * Eventos Comuns
 * ============================================================================
 *
 * Eventos Comuns irão ocorrer agora no final de cada ação independentemente
 * se a party inimiga esteja ou não viva. Com um posicionamento adequado das
 * tags de sequência de ações, você pode fazer com que o evento comum da
 * habilidade ocorra no meio de uma ação, também. Porém, tenha em mente que
 * se você forçar uma ação no meio de uma outra ação, o restante da lista de
 * sequência da primeira ação se tornará nulo e vazio em favor da nova
 * ação forçada.
 *
 * ============================================================================
 ...
 * ============================================================================
 *
 * Animações de Lançamento ajudam a fornecer dicas visuais a jogadores por
 * deixá-los saberem qual combatente vai realizar a ação, ou que tipo de
 * habilidade a ação será. Esse plugin habilita habilidades a terem
 * animações de lançamento que podem ser modificadas universalmente ou
 * customizada para cada habilidade individual.
 *
 * Tag de Habilidade:
 *   <Cast Animation: x>
 *   Estabelece a animação de lançamento da habilidade para animação ID x.
 *   Estabelecendo x para zero irá causar com que a habilidade não tenha
 *   nenhuma animação.
 *
 * ============================================================================
 * Mudando Sistemas de Batalha
 * ============================================================================
 *
 * Enquanto o jogador não estiver em batalha, você pode mudar o sistema de
 * batalha usando um Comando de Plugin. Com apenas esse Plugin, há apenas
 * um sistema de batalha incluso: o sistema de batalha padrão.
 *
 * Comando de Plugin:
 *   setBattleSys DTB Estabelece o sistema de batalha para Default Turn Battle.
 *
 * Outros plugins futuros podem incluir outros sistemas de batalhas que possam
 * utilizar o Battle Engine Core.
 *
 * ============================================================================
 * Ações da Vista de Lado
 * ============================================================================
 *
 * No sistema de batalha padrão do RPG MAKER MV, tanto as configurações da
 * vista de lado quanto da vista de frente não mostram contra-ataques,
 * ataques mágicos refletidos, ou nenhum caso de substituição para
 * membros da party. O Battle Engine Core fornece jogos que estão usando
 * pequenas quantidades de animações da configuração da vista de lado para
 * transmitir informações para o jogador num sentido mais visual.
 *
 * Reflexões Mágicas também mostrarão uma animação de reflexão para indicar
 * que o combatente tem propriedades de reflexão. Essa animação pode ser
 * mudada nos parâmetros, mas certos personagens, classes, inimigos,
 * armas, armaduras, e estados podem mostrar um tipo único de animação
 * para reflexão, se preferir.
 *
 * Tags de Personagem, Classe, Inimigo, Arma, Armadura, e Estado:
 *   <Reflect Animation ID: x>
 *   Muda a animação de reflexão do usuário para x. Isso irá fazer prioridade
 *   na seguinte ordem: Personagem, Classe, Inimigo, Arma, Armadura, Estado,
 *   Padrão.
 *
 * As vezes, você não quer que seus inimigos sejam capazes de se mover. Ou
 * você não quer que certos personagens sejam capazes de se mover. Eles
 * são estacionários por qualquer razão. Para realizar isso, você pode
 * usar esse tag para proibir o combatente de se mover.
 *
 * Tags de Personagem, Classe, Inimigo, Arma, Armadura, e Estado:
 *   <Sprite Cannot Move>
 *   Previne o sprite do combatente de se mover. Isso irá fazer
 *   prioridade na seguinte ordem: Personagem, Classe, Inimigo, Arma,
 *   Armadura, e Estado. Se um inimigo for incapaz de se mover quando
 *   ele realiza uma ação, ele irá piscar em branco como se faz
 *   normalmente na vista de frente.
 *
 * ============================================================================
 * Anchor de Combatente de Vista de Lado Personalizada
 * ============================================================================
 *
 * Combatentes de vista de lado são geralmente centralizados horizontalmente,
 * e fumdamentados em seus pés. Porém, nem todos os spritesheets de combatentes
 * de vista de lado funcionam dessa forma. No evento de você ter um
 * combatente de vista de lado que não se conforma com esses padrões,
 * você pode "anchor" eles de uma forma diferente.
 *
 * Tags de Personagem:
 *   <Anchor X: y.z>
 *   <Anchor Y: y.z>
 *   Isso estabelece a localização do anchor para o combatente de vista
 *   de lado do personagem em y.z. Por padrão, o anchor X é 0.5 enquanto
 *   que o anchor Y é 1.0. Se você quiser que o anchor X fique um pouco
 *   mais para a esquerda, faça-o menos de 0.5. Faça-o mais de 0.5 para
 *   fazer come que o anchor X fique mais para a direita. Para
 *   aumentar o anchor Y, estabeleça o número do valor para menos de
 *   1.0. Continue ajustando até você achar aquela configuração
 *   de anchor perfeita.
 *
 * ============================================================================
 ...
 * ============================================================================
 *
 * O Yanfly Engine Plugins - Battle Engine Core inclui a capacidade de usar
 * sequências de ações personalizadas. Sequências de ações são instruções
 * básicas para o jogo para criar uma habilidade personalizada tanto
 * visualmente quanto mecanicamente. O Battle Engine Core, porém, vai
 * incluir apenas a mais básica de sequências de ações, então as
 * instruções de como criar uma sequência de ação personalizada iram ser
 * incluídas no arquivo Help em futuras extensões de plugin para esse
 * plugin.
 *
 * ============================================================================
 * Changelog
